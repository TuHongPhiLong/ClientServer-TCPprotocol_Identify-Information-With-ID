...
public class ServerController extends Component implements Initializable {
    ...
    private void setBtn_initServer() throws IOException {
        ...
        while (true) {
            try {
                ...
                System.out.println("Khỏi động luồng cho Client: " + clientNumber);
                clientNumber++;
                    ...
                    public void run() {
                        try {
                            ...
                            while (true) {
                                ...
                                idSubject = in1.readInt();

                                System.out.println(idSubject);
                                sendResultCheckIdSubject();//gửi thông tin của subject
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
    ...
    private void sendResultCheckIdSubject() {
        try {
            //mySocketSendSubject.sendSubject(resultCompare());

            out1.writeObject(resultCompareIdSubject());
            out1.flush();
    ...
    private void sendResultCheckAccount() {
        try {
            if (resultCompareAccount()) {
                out.writeObject(resultCompareAccount());
                out.flush();
                ...
                System.out.println("Khong nhan dien duoc Client");
                kiemTraTaiKhoan = false;
            }
            System.out.println("Da gui ket qua kiem tra account: " + resultCompareAccount());
